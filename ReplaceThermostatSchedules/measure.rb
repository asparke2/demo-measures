...
#start the measure
class ReplaceThermostatSchedules < OpenStudio::Ruleset::ModelUserScript
    ...
    if heating_sch or cooling_sch
      ...
      selected_zones.each do |zone|
        ...
        if thermostatSetpointDualSetpoint.empty?
          runner.registerError("Script Error - cannot find thermostat for thermal zone '#{zone.name}'.")
          next
        end
