...
#start the measure
class AustinTexasUtilityRates < OpenStudio::Ruleset::WorkspaceUserScript
  
  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return "Austin Texas Utility Rates"
  end
  ...
  def arguments(workspace)
    ...
    elec_chs << "Austin_Energy_Commercial_Secondary_Voltage_10kW_50kW"
    ...
    elec_tar.setDefaultValue("Austin_Energy_Commercial_Secondary_Voltage_10kW_50kW")
    args << elec_tar
    
    #make an argument for the gas tariff
    gas_chs = OpenStudio::StringVector.new    
    gas_chs << "Texas_Gas_Service_Commercial"   
    gas_tar = OpenStudio::Ruleset::OSArgument::makeChoiceArgument('gas_tar', gas_chs, true)
    gas_tar.setDisplayName("Select a Gas Tariff.")
    gas_tar.setDefaultValue("Texas_Gas_Service_Commercial")
    args << gas_tar

    #make an argument for the chw tariff
    chw_chs = OpenStudio::StringVector.new    
    chw_chs << "Austin_Downtown_Chilled_Water"  
    chw_tar = OpenStudio::Ruleset::OSArgument::makeChoiceArgument('chw_tar', chw_chs, true)
    chw_tar.setDisplayName("Select a Chilled Water Tariff.")
    chw_tar.setDefaultValue("Austin_Downtown_Chilled_Water")
    args << chw_tar
  ...
  def run(workspace, runner, user_arguments)
    ...
    chw_tar = runner.getStringArgumentValue("chw_tar",user_arguments)
    
    #import the tariffs
    [elec_tar,gas_tar,chw_tar].each do |tar|
    ...
    if not workspace.getObjectsByType("Timestep".to_IddObjectType).empty?
      ...
      timestep = workspace.addObject(OpenStudio::IdfObject.new("Timestep".to_IddObjectType)).get
      timestep.setString(0,"4")
      runner.registerInfo("created new timestep to 15 min to match the demand window of the tariffs")
    end
...
AustinTexasUtilityRates.new.registerWithApplication
