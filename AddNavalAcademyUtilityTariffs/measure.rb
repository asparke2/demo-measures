...
#start the measure
class AddNavalAcademyUtilityTariffs < OpenStudio::Ruleset::WorkspaceUserScript
  
  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return "AddNavalAcademyUtilityTariffs"
  end
  ...
  def run(workspace, runner, user_arguments)
    ...
    tar_path = OpenStudio::Path.new("#{File.dirname(__FILE__)}/resources/NavalAcademyBlendedTariffs.idf")
    ...
    if tar_file.empty?
      tar_path = OpenStudio::Path.new("#{File.dirname(__FILE__)}/NavalAcademyBlendedTariffs.idf")
      tar_file = OpenStudio::IdfFile::load(tar_path)
    end

    if tar_file.empty?
      runner.registerError("Unable to find the file NavalAcademyBlendedTariffs.idf")
      return false
    else
...
AddNavalAcademyUtilityTariffs.new.registerWithApplication