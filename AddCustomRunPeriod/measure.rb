#start the measure
class UseCustomRunPeriod < OpenStudio::Ruleset::WorkspaceUserScript
  
  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return "UseCustomRunPeriod"
  end
  ...
  def arguments(workspace)
    ...
    #make an argument for the boiler efficiency
    start_year = OpenStudio::Ruleset::OSArgument::makeStringArgument("start_year",true)
    start_year.setDisplayName("Start Year")
    start_year.setDefaultValue("2012")
    args << start_year
    ...
    #make an argument for the boiler efficiency
    end_year = OpenStudio::Ruleset::OSArgument::makeStringArgument("end_year",true)
    end_year.setDisplayName("End Year")
    end_year.setDefaultValue("2012")
    args << end_year
  ...
  def run(workspace, runner, user_arguments)
    ...
    runner.registerInfo("added custom runperiod named #{run_period.name}")
...
UseCustomRunPeriod.new.registerWithApplication
