      ...
      supply_components.each do |supply_component|
        ...
        if not hVACComponent.empty?
          ...
          if is_fan
            ...
            fan_eff = motor_eff*initial_fan_efficiency/initial_motor_efficiency
            if fan_eff > 100.0
              fan_eff = 100.0
              runner.registerWarning("Requested total efficiency of #{fan_eff}% for #{hVACComponent.name} is not possible. Setting total efficiency to 100%.")
            elsif fan_eff < 10.0
              fan_eff = 10.0
              runner.registerWarning("Requested total efficiency of #{fan_eff}% for #{hVACComponent.name} is not possible. Setting total efficiency to 10.0%.")
            elsif fan_eff > 96.0
              runner.registerWarning("Requested total efficiency for #{hVACComponent.name} is greater than 96%.")
            end
            
            runner.registerInfo("Changing the motor efficiency from #{initial_motor_efficiency*100}% to #{motor_eff}% for '#{hVACComponent.name}' and total efficiency from #{initial_fan_efficiency*100}% to #{fan_eff}% on loop '#{loop.name}.'")
            initial_motor_efficiency_values << initial_motor_efficiency
            hVACComponent.setFanEfficiency(fan_eff*0.01)
            hVACComponent.setMotorEfficiency(motor_eff*0.01)
          else
            initial_motor_efficiency = hVACComponent.motorEfficiency
            runner.registerInfo("Changing the motor efficiency from #{initial_motor_efficiency*100}% to #{motor_eff}% for '#{hVACComponent.name}' on loop '#{loop.name}.'")
            initial_motor_efficiency_values << initial_motor_efficiency
            hVACComponent.setMotorEfficiency(motor_eff*0.01)
          end
