    ...
    loops.each do |loop|
      ...
      supply_components.each do |supply_component|
        ...
        if not hVACComponent.empty?
          ...
          initial_fan_efficiency = hVACComponent.fanEfficiency
          initial_fan_efficiency_values << initial_fan_efficiency
          
          target_motor_efficiency = initial_motor_efficiency + motor_eff * 0.01
          if target_motor_efficiency > 1.0
            target_motor_efficiency = 1.0
          ...
          elsif target_motor_efficiency < 0.1
            target_motor_efficiency = 0.1
            runner.registerWarning("Requested efficiency of #{target_motor_efficiency*100}% for #{hVACComponent.name} is not possible. Setting motor efficiency to 0.1%.")
          elsif target_motor_efficiency > 0.96
            runner.registerWarning("Requested efficiency for #{hVACComponent.name} is greater than 96%.")
          end
          
          target_fan_efficiency = target_motor_efficiency*initial_fan_efficiency/initial_motor_efficiency
          if target_fan_efficiency > 1.0
            target_fan_efficiency = 1.0
            runner.registerWarning("Requested total efficiency of #{target_fan_efficiency*100}% for #{hVACComponent.name} is not possible. Setting total efficiency to 100%.")
          elsif target_fan_efficiency < 0.1
            target_fan_efficiency = 0.1
            runner.registerWarning("Requested total efficiency of #{target_fan_efficiency*100}% for #{hVACComponent.name} is not possible. Setting total efficiency to 10%.")
          elsif target_fan_efficiency > 0.96
            runner.registerWarning("Requested total efficiency for #{hVACComponent.name} is greater than 96%.")
          end

          hVACComponent.setFanEfficiency(target_fan_efficiency)
          hVACComponent.setMotorEfficiency(target_motor_efficiency)
          runner.registerInfo("Changing the motor efficiency from #{initial_motor_efficiency*100}% to #{target_motor_efficiency*100}% and total efficiency from #{initial_fan_efficiency*100}% to #{target_fan_efficiency*100}% for '#{hVACComponent.name}' on loop '#{loop.name}.'")
